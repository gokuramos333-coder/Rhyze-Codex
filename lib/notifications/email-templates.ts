...
export const emailTemplateCatalog = {
  ...
  WAITLIST_SPOT_AVAILABLE: {
    label: 'Waitlist spot available', category: 'Bookings', trigger: 'When an open spot is announced to the five-person waitlist for first-come booking',
    subject: (p) => `A spot opened for ${text(p, 'className', 'your Rhyze class')}`, sample: booking,
    present: (p) => ({ eyebrow: 'Waitlist opening', headline: 'A spot just opened', greeting: `Hi ${text(p, 'name', 'Rhyzer')},`, paragraphs: [`A cancellation opened a spot in ${text(p, 'className', 'the class or event')} on ${text(p, 'classDate', 'the scheduled date')} at ${text(p, 'classTime', 'the scheduled time')}.`, 'This spot is first come, first served for the waitlist. The first waitlisted member who completes booking gets the spot.'], facts: [{ label: 'Class/Event', value: text(p, 'className', 'Rhyze class or event') }, { label: 'When', value: `${text(p, 'classDate', '')} · ${text(p, 'classTime', '')}` }, { label: 'Instructor', value: text(p, 'instructorName', 'Rhyze instructor') }], cta: { label: 'Claim this spot', href: text(p, 'claimUrl', '/member/bookings') } }),
  },
  CLASS_UPDATE: {
