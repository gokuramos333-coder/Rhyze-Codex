...
BEGIN
  FOR merge_record IN
    SELECT * FROM (VALUES
      ('julie-reese@rhyze.local', 'gritandgracefitnessnj@gmail.com', 'Julie Reese'),
      ('tricia-johnsen@rhyze.local', 'tcjdancefit@gmail.com', 'Tricia Johnsen')
  ...
  LOOP
    ...
    IF local_user_id IS NULL OR real_user_id IS NULL THEN
      RAISE EXCEPTION 'Inactive-local instructor class assignment merge expected both local and real accounts for %', merge_record.instructor_name;
    END IF;
    ...
    INSERT INTO "AuditLog" (
      ...
      'rhyze-inactive-local-instructor-assignment-merge-20260826-' || regexp_replace(merge_record.instructor_name, '[^A-Za-z0-9]+', '-', 'g'),
      NULL,
