...
export default async function TransferPage({ params, searchParams }: { params: { occurrenceId: string }; searchParams: { booking?: string; error?: string } }) {
  ...
  return (
    <>
      ...
      <p className="mt-3 text-rhyze-black/55">Move from {booking.occurrence.template.name} on {booking.occurrence.startAt.toLocaleString()}.</p>
      {searchParams.error && <p className="mt-5 border-l-4 border-rhyze-coral bg-white p-4 font-bold text-rhyze-coral">{searchParams.error === 'payment' ? 'The $10 transfer fee could not be charged. The original booking was not moved.' : 'That destination is no longer eligible or available.'}</p>}
      <form action={transferBookingAction} className="mt-8 grid gap-4 border-t-4 border-rhyze-gold bg-white p-6">
        ...
        <label className="grid gap-2"><span className="text-xs font-black uppercase tracking-widest">Destination class</span><select name="destinationId" required className="min-h-14 border px-3"><option value="">Choose an eligible class</option>{destinations.filter((item) => item._count.bookings < item.capacity).map((item) => <option key={item.id} value={item.id}>{item.startAt.toLocaleString()} · {item.template.name} · {item.instructor?.name || 'TBA'}</option>)}</select></label>
        ...
        <p className="text-xs text-rhyze-black/55">More than 6 hours: free. 6–2 hours: $10 saved-card charge unless VIP. 2 hours or less: transfer blocked.</p>
